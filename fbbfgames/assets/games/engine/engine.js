/*
 * FBBF mini-game engine — shared runtime for every game.
 *
 * A game's game.js calls FBBFGame.start({ slug, title, instructions, onInit,
 * onUpdate, onDraw }) once its DOM (from the shared template) is in place.
 * The engine owns: the render loop, canvas sizing, keyboard/pointer input,
 * score display, high-score persistence, and the start/game-over overlays.
 * The per-game code only ever touches game state and drawing.
 */
(function () {
	"use strict";
	...
	function start(config) {
		...
		const input = {
			...
			pointerX: 0,
			pointerY: 0,
			// Discrete tap events, queued so a quick click can't land between
			// two animation frames and get missed. Drain it in onUpdate.
			taps: [],
		};
